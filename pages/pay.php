...
$bill_id = (int)($_GET['bill'] ?? $_GET['id'] ?? 0);
$msg = '';
...
$waiting_verify = $bill['status'] === 'waiting';

// รับสลิป
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$already_paid && !$waiting_verify) {
    $slip_path = '';
    ...
    $conn->query("INSERT INTO payment 
        ...
        VALUES ($bill_id, $total, NOW(), 'qr', '$slip_path', '$note', 1, 0)");

    // อัปเดต bill เป็น waiting
    $conn->query("UPDATE bill SET status = 'waiting' WHERE id_listbill = $bill_id");
...
<div class="card">
    ...
    <div class="card-body">
        ...
        <?php if ($msg === 'success'): ?>
        <!-- Waiting Verify State (just submitted) -->
        <div class="success-box">
            <div class="success-icon">⏳</div>
            <div class="success-title" style="color:#e67e22;">รอการยืนยัน</div>
            <div class="success-text">
                ส่งสลิปเรียบร้อยแล้ว!<br>
                เจ้าหน้าที่กำลังตรวจสอบการโอนเงิน<br>
                <span style="color:#aaa;font-size:12px;">ภายใน 24 ชั่วโมง ขอบคุณครับ 🙏</span>
            </div>
        </div>

        <?php elseif ($waiting_verify): ?>
        ...
        ">
            ...
            <div style="font-size:18px;font-weight:800;color:#e67e22;">รอการยืนยันจากเจ้าหน้าที่</div>
            <div style="font-size:13px;color:#888;margin-top:8px;line-height:1.7;">
                ระบบได้รับสลิปของคุณแล้ว<br>
                เจ้าหน้าที่กำลังตรวจสอบการโอนเงิน<br>
                <span style="font-size:12px;color:#bbb;">กรุณารอการยืนยัน ขอบคุณครับ 🙏</span>
            </div>
        </div>
